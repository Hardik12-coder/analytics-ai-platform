...
def _call_llama(stats: dict[str, StatsValue], context: str | None, model: str | None) -> str:
    """Call a hosted Llama model through Together AI.

    This deliberately uses hosted inference instead of self-hosting model
    weights. The same safe prompt-construction rules apply.
    """

    api_key = _required_env("TOGETHER_API_KEY")
    selected_model = model or os.getenv("TOGETHER_MODEL") or DEFAULT_MODELS["llama"]

    try:
        from together import Together
    except ImportError as exc:
        raise RuntimeError("Install the 'together' package to use hosted Llama.") from exc
    ...
    return _extract_chat_completion_text(response, "Together/Llama")
...
def _extract_chat_completion_text(response: Any, provider_name: str) -> str:
